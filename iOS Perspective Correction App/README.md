...
Debug Simulator launches with `--verify-camera-ui` open the camera controls without reading API keys (camera availability still depends on hardware). `--verify-proportions` use a synthetic calibrated card; add `--without-calibration` to verify the manual fallback. `--verify-dewarp` shows a tilted synthetic card for checking the perspective animation and Replay control. `--verify-processing` shows that photo under the processing indicator for three seconds before the dewarp animation, for checking continuity and cancellation. These fixtures make no API calls and do not read keys. Normal launches use the real camera/library/Photoroom flow.
